...
static FlashMemConfig_t *_FlashMem;
static PowerConfig_t *_Power;
...
void Power_Configuration(PowerConfig_t *pwr_conf){
	...
	if(_Power -> power_clockenable == true) (RCC -> APB1ENR |= RCC_APB1ENR_PWREN);
	else  (RCC -> APB1ENR &=~ RCC_APB1ENR_PWREN);
	/* SET VOLT SCALE
	 *  - SCALE 2 FOR EVICE CLOCK BELOW MAX FREQUENCY */
	if(_Power -> power_voltscale2) PWR -> CR &=~ PWR_CR_VOS;
	else{
		PWR -> CR |= PWR_CR_VOS;
...
void FlashMem_Configuration(FlashMemConfig_t *flh_conf){
	...
	uint32_t latency= (uint32_t)(_FlashMem -> rcc_systemclock / 30000000U);
	if(_FlashMem -> rcc_systemclock == 30000000U  || _FlashMem -> rcc_systemclock == 60000000U  || _FlashMem -> rcc_systemclock == 90000000U
    || _FlashMem -> rcc_systemclock == 120000000U || _FlashMem -> rcc_systemclock == 150000000U || _FlashMem -> rcc_systemclock == 180000000U) latency -= 1;
	_FlashMem -> flash_latency = latency;
	tmpreg |= (uint32_t)(latency << FLASH_ACR_LATENCY_Pos);

	/* SET FLASH DATA CACHE */
	if(_FlashMem -> flash_datacache) tmpreg |= FLASH_ACR_DCEN;
	else tmpreg &=~ FLASH_ACR_DCEN;

	/* SET FLASH INSTUCTION CACHE */
	if(_FlashMem -> flash_instructioncache) tmpreg |= FLASH_ACR_ICEN;
	else tmpreg &=~ FLASH_ACR_ICEN;

	/* SET FLASH PREFETCH CACHE */
	if(_FlashMem -> flash_prefetch) tmpreg |= FLASH_ACR_PRFTEN;
