...
static void DS3231_Temp_Conv(void){
	uint8_t status = 0;
	uint8_t control = 0;
	_i2c -> Memory_Receive(DS3231_ADDRESS, 0x0F, 1, &status, 1);
	if (!(status & 0x04)){
		_i2c -> Memory_Receive(DS3231_ADDRESS, 0x0E, 1, &control, 1);
		_i2c -> Memory_Transmit(DS3231_ADDRESS, 0x0E, 1, (uint8_t *)(control|(0x20)), 1);
	}
}
...
void DS3231_SetTime(DS3231_Time_t time){
	...
	_i2c -> Memory_Transmit(DS3231_ADDRESS, 0x00, 1, set_time, 7);
}

void DS3231_GetTime(DS3231_Time_t *time){
	uint8_t get_time[7];

	_i2c -> Memory_Receive(DS3231_ADDRESS, 0x00, 1, get_time, 7);
...
float DS3231_GetTemp(void){
	...
	_i2c -> Memory_Receive(DS3231_ADDRESS, 0x11, 1, temp, 2);
