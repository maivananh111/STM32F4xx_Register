...
class LoRa{
	public:
		LoRa(GPIO_TypeDef *CSPort, uint16_t CSPin, GPIO_TypeDef *RSTPort, uint16_t RSTPin, GPIO_TypeDef *INTPort, uint16_t INTPin);

		bool Init(SPI<uint8_t> *spi, long frequency, uint8_t power, uint32_t interruptpriority);
		void Event_Register_Handler(void (*TxHandler)(void *arg), void (*RxHandler)(void *arg, uint8_t len));
	...
	private:
		...
		uint16_t _cs, _rst, _int;
		GPIO_TypeDef *_csport, *_rstport, *_intport;
		int 		_packetIndex = 0;
